...
int main(int argc, char* argv[]) {
  ...
  // The default device selector will select the most performant device.
	...
	try {
		...
		Sigma<ftypePML> Sigma(q, Nx, Ny, Nz, delta_x, delta_y, delta_z);
		...
		for (int it = 0; it < Nt; ++it) {
            ...
            q.submit([&](sycl::handler& h) {
                h.parallel_for(range<2>(Ny, Nz), [=](auto index) {
                    ...
                    kernel_cube.Ey(offset + index_start_x, j + index_start_y, k + index_start_z) += A * exp(-(t1 - t0_x) * (t1 - t0_x) / (tp_x * tp_x))
                        * exp(-(y2 - y00) * (y2 - y00) / (ay_transverse * ay_transverse)) *
                        sin(omega_ * t1 - k_ * x1); // sin(phase), phase = omega * t - k * x
                    kernel_cube.Bz(offset + index_start_x, j + index_start_y, k + index_start_z) += A * exp(-(t2 - t0_x) * (t2 - t0_x) / (tp_x * tp_x))
                        * exp(-(y2 - y00) * (y2 - y00) / (ay_transverse * ay_transverse)) *
            ...
            q.submit([&](sycl::handler& h) {
				h.parallel_for(range<3>(Nx + 2 * delta_x, Ny + 2 * delta_y, delta_z), [=](auto index) {
					int i = index[0] + 1;
					int j = index[1] + 1;
					int k = index[2] + 1;
            ...
            q.submit([&](sycl::handler& h) {
				h.parallel_for(range<3>(Nx + 2 * delta_x, delta_y, Nz + delta_z), [=](auto index) {
					int i = index[0] + 1;
					int j = index[1] + 1;
					int k = index[2] + delta_z + 1;
            ...
            q.submit([&](sycl::handler& h) {
				h.parallel_for(range<3>(delta_x, Ny + delta_y, Nz + delta_z), [=](auto index) {
					int i = index[0] + 1;
					int j = index[1] + delta_y + 1;
					int k = index[2] + delta_z + 1;
            ...
            q.submit([&](sycl::handler& h) {
				h.parallel_for(range<3>(Nx, delta_y, Nz + delta_z), [=](auto index) {
					int i = index[0] + delta_x + 1;
            ...
            q.submit([&](sycl::handler& h) {
				h.parallel_for(range<3>(Nx, Ny, delta_z), [=](auto index) {
					int i = index[0] + delta_x + 1;
					int j = index[1] + delta_y + 1;
					int k = index[2] + Nz + delta_z + 1;
            ...
            //edge from 0,0,0 by XZ
			q.submit([&](sycl::handler& h) {
				h.parallel_for(range<3>(Nx + 2 * delta_x, delta_y, delta_z), [=](auto index) {
					int i = index[0] + 1;
					int j = index[1] + 1;
					int k = index[2] + delta_z + 1;
            ...
            //edge from 0,0,0 by YZ
			q.submit([&](sycl::handler& h) {
				h.parallel_for(range<3>(delta_x, delta_y, delta_z), [=](auto index) {
					int i = index[0] + 1;
					int j = index[1] + delta_y + 1;
					int k = index[2] + delta_z + 1;
